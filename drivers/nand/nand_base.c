...
//==========================================================
extern int use_bbt;
extern int bbt_count;
extern unsigned long bbt_page[];
extern unsigned long map_page[];
//==========================================================
#endif
...
//	if((command == NAND_CMD_READ0)||(command == NAND_CMD_READ1)||(command == NAND_CMD_PAGEPROG)||(command == NAND_CMD_ERASE1)||(command == NAND_CMD_ERASE2))
//		printf("** NAND CMD: 0x%X, col: %d, page: 0x%X\n", command, column, page_addr);
	/*
			    ...
			    int column, int page_addr)
{
	...
	if(use_bbt == 1)
	{
		...
		{
			if((page_addr & ~0x3F) == bbt_page[i])
			{
				//j = page_addr;
				page_addr = (map_page[i] | (page_addr & 0x3F));
				//printf("mapped page address 0x%08X ==> 0x%08X\n", j, page_addr);
				break;
			}
		}
	}
