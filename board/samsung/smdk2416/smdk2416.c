...
{
#if 1 
	SMBIDCYR1_REG = 8;		//10;			//Bank1 Idle cycle ctrl.
	SMBWSTWRR1_REG = 8;		//14;			//Bank1 Write Wait State ctrl.
	SMBWSTOENR1_REG = 3;	// 2;			//Bank1 Output Enable Assertion Delay ctrl.     Tcho?
	SMBWSTWENR1_REG = 3;	//2;			//Bank1 Write Enable Assertion Delay ctrl.
	SMBWSTRDR1_REG = 8;		//14;			//Bank1 Read Wait State cont. = 14 clk	    Tacc?
#endif
	SMBCR1_REG |=  (2<<22);
...
int board_init(void)
{
	...
	gpdat &= ~((1<<1) | (1<<5) | (1<<7));
	writel(gpdat, GPADAT);
	writel(0x15555555, GPKCON);
	writel(0x00008000, GPKDAT);
#else
	gpcon = readl(GPACON);
	gpcon &= ~((1<<1) | (1<<5) | (1<<7));
	writel(gpcon, GPACON);
